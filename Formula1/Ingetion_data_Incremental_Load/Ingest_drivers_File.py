...
drivers_transformed_df= driver_df.withColumnRenamed("driverid", "driver_id").withColumnRenamed("driverRef", "driver_ref").withColumn("name", concat(col("name.forename"), lit(" "), col("name.surname"))).transform(add_ingestion_data).withColumn("data_source", lit(v_data_source)).withColumn("file_date", lit(v_file_date))
